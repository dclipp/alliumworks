...
export class ResponsiveTinyDirective implements AfterViewInit, OnDestroy {
    public ngAfterViewInit(): void {
        const when = this._ref.nativeElement.getAttribute('rx-tiny');
        if (!!when) {
            const c = ResponsiveTinyDirective.parseCondition(when);
            this._handle = ResponsiveViewService.bindResponsiveCondition(c.group, c.state, 'tiny', () => {
