...
export class MemoryExplorerDataHelper {
    ...
    private emitQuadChanges(): void {
        ...
        Object.keys(this._hash).forEach(k => {
            const quadStore = (this as any)[`_${k}`] as MemoryExplorerQuadStore;
            const calculatedHash = quadStore.collection.hash();
            if ((this._hash as any)[k] !== calculatedHash) {
                (changes as any)[k] = quadStore.collection;
                (this._hash as any)[k] = calculatedHash;
                anyChanged = true;
            }
        })
